...
风险权重 \(\lambda{}\) & \(0\leq\lambda{}\leq1\) & 控制期望利润与下尾利润的偏好 \\
...
弹性稳定条件 & \(\sum{}_{h\neqi}\lvert e_{ih}\rvert\leq\lvert e_{ii}\rvert\) & 防止交叉效应支配自身价格效应 \\
...
递推。每个情景只抽取一个 \(S_\omega{}~\chi{}^{2}(ν)\)，由该情景内全部变量和七个年份共享。基准取 \texttt{ν=5}、载荷强度 1、\(\rho{}=0.5\)；相关强度和持续性按弱中强档扫描。审计对固定为所有 \(|\tau{}*|\geq0.10\) 的非对角无序对，加上同一维度相邻年份对。
